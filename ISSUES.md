...
- [ ] **Export functions return empty strings (getMarkdown/getHtml)**. When the user types content into the editor and then calls `editor.getMarkdown()` or `editor.getHtml()`, an empty string is returned. Root cause: `this.blocks` is populated only once during `init()` from the initial content. After that, typed content exists only in the DOM but is never synced back to the block objects. `block.toMarkdown()` returns stale `_content` from construction time. Discovered during E2E testing — see `e2e/editor.spec.js` tests marked `@bug`.
- [ ] **Code block markdown trigger (```) not working**. Typing three backticks + Enter does not create a code block. Root cause: `CodeBlock.applyTransformation()` is intentionally a no-op (comments say "Don't call Toolbar.code() to avoid circular dependency"). The conversion sets the block's text but never transforms the DOM element into an actual code block. This also affects toolbar-based code block conversion.
- [ ] **Quote block markdown trigger (>) not working**. Typing `>` + space does not create a blockquote block. Root cause: the browser encodes `>` as `&gt;` in contenteditable, so the trigger pattern `> ` doesn't match the HTML entity `&gt; `.
- [ ] **test-page.html had wrong editor ID**. The test page used `id: 'test-editor'` but Editor.init() hardcodes `editorContainer.id = 'editor'`, then tries `document.getElementById(options.id)` which returned null. Fixed by changing to `id: 'editor'`. The underlying issue is that Editor.init() ignores `options.id` when creating the DOM element.
- [ ] **test-page.html had DOCTYPE typo**. `<!DOCOTYPE html>` instead of `<!DOCTYPE html>`. Fixed.
