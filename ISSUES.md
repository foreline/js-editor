...
- [ ] **Editor.setCurrentBlock is called multiple times** When a cursor is placed at a block, the `Editor.setCurrentBlock()` method is called multiple times unnecessarily, leading to performance issues. This should be optimized to ensure it's only called when the current block actually changes.
