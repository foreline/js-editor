...
- [x] **Block conversion using special key sequences not working**. When user creates a new default (paragraph) block and hits the special key sequences i.e. `# `, `- `, `* `, `1 `, `1. ` which should convert the paragrash block to H1, UnOrderedList, OrderedList Blocks, that behaviour is not observed, but expected. 
