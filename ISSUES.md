...
- [x] **Markdown trigger for empty block not working** When hitting markdown trigger key sequence, i.e. `# `, `- `, `* `, `1 `, `1. ` the empty block should convert to the corresponding block type. Now the block tries to convert, but then just clears its contents and resets to default block type. ✅ **FIXED** - The issue was in the `convertBlockType` method where the block content was cleared before `applyTransformation()` was called, causing the transformation method to read empty content. Fixed by setting the block content to the remaining content (without trigger) before applying transformation, ensuring proper conversion even for trigger-only inputs.
